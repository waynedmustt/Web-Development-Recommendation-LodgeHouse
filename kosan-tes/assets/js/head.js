/* main_website/head.js */

/* TrueCar.ABTest */
if(!window.TrueCar){throw new Error("TrueCar.ABTest requires TrueCar");}TrueCar.ABTest=TrueCar.ABTest||{VERSION:"0.0.2.4",init:function(d,a,c){var b="ABCDEFGHIJKLMNOPQRST";this.test_id=d;this.test_cases={};this.monthsArray=["01","02","03","04","05","06","07","08","09","10","11","12"];this.runTest=function(){};this.checkTest=function(j){var f=this.getUserKey(this.test_id);if(j==undefined){var h=0;for(var e=0;e<a.length;e++){var g=(a[e]*b.length/100);if(b.substring(h,g+h).indexOf(f)>=0){j=e;break;}h+=g;}}if(j&&this.test_cases[j]!=undefined){this.test_case=this.test_cases[j];this.runTest=function(){document.write(unescape(this.test_case.testContent));};document.write("<style>#abtest_"+this.test_id+"_control.control {display:none;}</style>");}if(c){this.setOmnitureVars(c[0],c[1]);}};this.setOmnitureVars=function(h,f){if(!this.test_case){this.test_case={testCase:"A"};}var g=new Date();var e="abtest_"+g.getFullYear()+this.monthsArray[g.getMonth()]+"_"+this.test_id+"_"+this.test_case.testCase;s["prop"+h]=e;s["eVar"+f]=e;};this.getUserKey=function(e){if(this.readCookie("random_id")==null){this.setCookie(this.randomString(40));}var f=this.readCookie("random_id");if(!f){f=this.randomString(40);}return f.substring(e-1,e);};this.randomString=function(h){var f="";for(var g=0;g<h;g++){var e=Math.floor(Math.random()*b.length);f+=b.substring(e,e+1);}return f;};this.setCookie=function(f){var g="random_id";var e=new Date(new Date().getTime()+365000*86400);document.cookie=g+"="+escape(f)+"; domain=truecar.com; expires="+e.toUTCString()+"; path=/";};this.readCookie=function(f){var h=f+"=";var e=document.cookie.split(";");for(var g=0;g<e.length;g++){var j=e[g];while(j.charAt(0)==" "){j=j.substring(1,j.length);}if(j.indexOf(h)==0){return j.substring(h.length,j.length);}}return null;};}};

/* TrueCar.Ads */
if(!window.TrueCar){throw new Error("TrueCar.Ads requires TrueCar");}TrueCar.Ads=TrueCar.Ads||{VERSION:"0.9.1",data:{queue:[],ord:Math.random()*10000000000000000},refresh:function(f){if(TrueCar.QueryString&&TrueCar.QueryString.data.adblock){return false;}var e="";if(this.data.queue.length){for(var c=1,a=this.data.queue.length;c<a;c++){if(f&&c!=f){continue;}try{if(segQS.length<=1){segQS="btseg=;";}e=segQS.replace(/;/g,"|").replace(/=/g,"_");}catch(b){}var d=document.getElementById(this.data.queue[c][0]);d.src=this.data.queue[c][1]+segQS+this.data.queue[c][2]+e+this.data.queue[c][3]+this.data.ord+"?";d.setAttribute("width",this.data.queue[c][4]);if((this.data.queue[c][4]==300)&&(this.data.queue[c][5]==250)){d.setAttribute("height",600);}else{d.setAttribute("height",this.data.queue[c][5]);}if(window.console){console.log(this.data.queue[c][1]+segQS+this.data.queue[c][2]+e+this.data.queue[c][3]+this.data.ord+"?");}}}}};

/* Omniture SiteCatalyst H.20.3. */
/* SiteCatalyst code version: H.20.3.
Copyright 1997-2009 Omniture, Inc. More info available at
http://www.omniture.com */

var s_account=omnitureRSID ? omnitureRSID : "truecardev";
...
s.linkInternalFilters="javascript:,truecar.com";
s.linkLeaveQueryString=false;
s.linkTrackVars="None";
s.linkTrackEvents="None";
s.trackingServer=omnitureCookieDomain;
s.trackingServerSecure="s"+omnitureCookieDomain;
...
s.eVar1 = s.getQueryParam('int_cid');
s.prop15=s.getNewRepeat();
s.eVar15=s.getValOnce(s.prop15,'s_evar15',0);
s.prop16=s.getDaysSinceLastVisit('s_lv');
s.eVar16=s.getValOnce(s.prop16,'s_evar16',0);
s.prop17=s.getVisitNum();
s.eVar17=s.getValOnce(s.prop17,'s_evar17',0);

	/* Set Time Parting Variables */
	var theDate=new Date();
	var currentYear=(theDate.getFullYear());
	s.prop18=s.getTimeParting('h','-8',currentYear); 	// Set hour of day
	s.eVar18=s.getValOnce(s.prop18,'s_evar18',0);
	s.prop19=s.getTimeParting('d','-8',currentYear);	// Set Day of week
	s.eVar19=s.getValOnce(s.prop19,'s_evar19',0);
	s.prop20=s.getTimeParting('w','-8',s.currentYear); // Set Weekend/Weekday
